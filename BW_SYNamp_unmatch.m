function [spike_distribution, BW, rate_peak, ILD50]=...
    BW_SYNamp_unmatch(synapse_amp_In,BW_type,cf_shift)
% change the # of inhibitory inputs
% by CCG @ 2023-03-14
...
if strcmp(BW_type, 'ILD')
    ...
    mean_In=3.5*synapse_amp_In; std_In=1; % sigma=Scale parameter of logarithmic values
    E_strength = 1+flip(mean_Ex*pdf('LogLogistic', x, 1, std_Ex));
    I_strength = 1+flip(mean_In*pdf('LogLogistic', x, 1+cf_shift, std_In));
elseif strcmp(BW_type, 'Hz')
    spike_num_Poi = 10 ; % Number of Possion spikes
    mean_Ex=1.5; std_Ex=0.5;
    mean_In=3.5*synapse_amp_In; std_In=1;%raw
    E_strength = 1+transpose(mean_Ex*normpdf(x,3,std_Ex)); %0.5sd in paper
    I_strength = 1+transpose(mean_In*normpdf(x,3+cf_shift,std_In)); %1.0sd in paper---raw
end
nreps = numel(x);
step=.0001; % [S]
